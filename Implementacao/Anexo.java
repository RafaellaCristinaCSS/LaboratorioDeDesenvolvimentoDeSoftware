...
public class Anexo {
    ...
    public void FiltrarAquivoPorChaveDoElemento(String chaveDesejada, String valorDesejado, String nomeDoAquivo) {
        try (BufferedReader reader = new BufferedReader(new FileReader(nomeDoAquivo+".txt"))) {
            String linha;
            while ((linha = reader.readLine()) != null) {
                if (linha.trim().startsWith("{") && linha.trim().endsWith("}")) {
                    ...
                    String[] pares = linha.split(",");
                    String chaveAtributo = null;
                    for (String par : pares) {
                        String[] partes = par.split(":");
                        if (partes.length == 2) {
                            String chave = partes[0].trim();
                            String valor = partes[1].trim();
                            String stringChaveDesejada = "'"+ chaveDesejada +"'";
                            if (stringChaveDesejada.equals(chave)) {
                                chaveAtributo = valor.replace("\"", "").trim();
                            }
                        }
                    }
                    
                    if (chaveAtributo != null && chaveAtributo.equals("'"+valorDesejado+"'")) {
                        System.out.println("Linha encontrada:");
                        System.out.println("{" + linha + "}");
                        break; 
                    }
